...
const genericController =(service, uniqueField, isAdmin)=>{
  // Methods:
   function responder (res, status, success, message = null, results = null) {
...
return {
  ...
  create : catchController(async (req, res) => {
    const data = req.body
    const response = await service.create(data, uniqueField)
    return responder(res, 201, true, response.message, response.results)
  }),

  getAll : catchController(async (req, res) => {
    const queryObject = req.query
    const response = await service.getAll(queryObject,isAdmin)
    return responder(res, 200, true, response.message, response.results)
  }),

  getById : catchController(async (req, res) => {
    const { id } = req.params
    const response = await service.getById(id, isAdmin)
    return responder(res, 200, true, response.message, response.results)
  }),
...
const baseRepository = (Model, dataEmpty = null) =>{
  ...
  async create (data, uniqueField) {
      ...
      if (existingRecord) {
        throwError(\`This \${Model.name.toLowerCase()} \${uniqueField || 'entry'} already exists\`, 400)
      }
  ...
  async getAll(searchField= null, search = null, order, sortBy, page = 1, limit = 10, isAdmin = false) {
    ...
    const { rows: resModel, count: totalCount } = await Model
    ...
    if (resModel.length === 0) {
      if (dataEmpty) {
        resModel.push(dataEmpty);
      } else {
        throwError(\`This \${Model.name.toLowerCase()} \${searchField || 'entry'} does not exist\`, 404);
      }
    }
  ...
  async getOne (data, uniqueField, isAdmin = false) {
      ...
      const existingRecord = await Model.scope(isAdmin ? 'allRecords' : 'enabledOnly').findOne({ where: whereClause })
      if (!existingRecord) {
        throwError(\`This \${Model.name.toLowerCase()} name do not exists\`, 404)
      }
  ...
  async getById (id, isAdmin = false) {
      ...
      const existingRecord = await Model.scope(isAdmin ? 'allRecords' : 'enabledOnly').findByPk(id)
      if (!existingRecord) {
        throwError(\`This \${Model.name.toLowerCase()} name do not exists\`, 404)
      }
      return existingRecord
  ...
  async update (id, data) {
    ...
    if (!dataFound) {
      throwError(\`\${Model.name} not found\`, 404)
    }
  ...
  async delete (id) {
    const dataFound = await Model.findByPk(id)
    if (!dataFound) {
      throwError(\`\${Model} not found\`, 404)
    }
    let deletedRecord = Model.name.toLowerCase()
    await dataFound.destroy(id)
    return \`\${deletedRecord} deleted successfully\`
  }
 }
}
...
const generalService =  (Repository, fieldName, parserFunction = null, useImage = false, deleteImages = null) =>{
...
return {
  ...
  async create (data, uniqueField = null) {
    try {
      const newRecord = await Repository.create(data, uniqueField)
  ...
  async getAll (queryObject, isAdmin = false,) {
    ...
    const { searchField, page, limit, search, filters,sortBy,order} = queryObject
  
    const response = await Repository.getAll(searchField, search, filters, sortBy, order, page, limit, isAdmin)

    const dataParsed = parserFunction? response.data.map(dat => parserFunction(dat)) : response.data
  ...
  async getById (id, isAdmin = false) {
    const data = await Repository.getById(id, isAdmin)
  ...
  async update (id, newData) {
    ...
    const dataFound = await Repository.getById(id, newData)

    if (useImage && dataFound.picture && dataFound.picture !== newData.picture) {
      imageUrl = dataFound.picture
      deleteImg = true
    }

    const upData = await Repository.update(id, newData)

    if (deleteImg) {
      await handleImageDeletion(imageUrl)
    }
  ...
  async delete (id) {
    ...
    let imageUrl = ''
    try {
      const dataFound = await Repository.getById(id)
      const dataReg = dataFound[fieldName]
      if(useImage){  
        imageUrl = dataFound.picture 
        deleteImg = true
      }
      ...
      if(deleteImg){await handleImageDeletion(imageUrl)}

      return { message: \`\${fieldName} deleted successfully\`, data: dataReg }
    } catch (error) {
      throw error
    }
  },
...
const tests = baseRepository(Landing) //Se necesita tener al menos una tabla declarada en la DB
const failed = baseRepository(User, info.dataEmpty) //Debe ser una tabla diferente 

describe('BaseRepository tests (Functions)', () => {
 beforeAll(async () => {
    await resetDatabase()
  })
...
const testing = baseRepository(Landing)

// repository, fieldName(string), parserFunction(function), useImage(boolean), deleteImages(function)
const serv = generalService(testing, 'Landing', null, true, fns.deletFunctionFalse)
const servCache = generalService(testing, 'Landing', info.cleanData, false, fns.deletFunctionTrue)
const servParse = generalService(testing, 'Landing', info.cleanData, true, fns.deletFunctionTrue)

describe('Test unitarios de la factory function GeneralService: CRUD.', () => {
 beforeAll(async () => {
    await resetDatabase()
  })
  ...
  describe('Metodos "GET". Retornar servicios o un servicio.', () => {
    it('Metodo "getAll": deberia retornar un arreglo con los servicios', async () => {
      ...
      const queryObject = {
        searchField: 'name'}
      const response = await servParse.getAll(queryObject, false)
      //console.log('A ver el get', response)
   ...
   describe('Metodo "delete".', () => {
    ...
    it('deberia arrojar un error si falla la eliminacion de imagenes', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(Error)
        expect(error.status).toBe(500)
        expect(error.message).toBe('Error processing ImageUrl: https://metalogo.com.ar')
      }
    })
  })
})
